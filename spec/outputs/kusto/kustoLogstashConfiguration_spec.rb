...
describe LogStash::Outputs::KustoInternal::KustoLogstashConfiguration do
  ...
  let(:kusto_ingest_base) do
    LogStash::Outputs::KustoInternal::KustoIngestConfiguration.new(ingest_url, database, table, json_mapping)
  end
  let(:kusto_auth_base) do
    LogStash::Outputs::KustoInternal::KustoAuthConfiguration.new(app_id, app_key, app_tenant, managed_identity, cliauth)
  end
  let(:kusto_proxy_base) do
    LogStash::Outputs::KustoInternal::KustoProxyConfiguration.new(proxy_host, proxy_port, proxy_protocol,
                                                                  proxy_aad_only)
  end
  let(:kusto_flush_config) do
    LogStash::Outputs::KustoInternal::KustoFlushConfiguration.new(max_items, plugin_flush_interval, max_batch_size,
                                                                  process_failed_batches_on_startup)
  end
  let(:kusto_upload_config) do
    LogStash::Outputs::KustoInternal::KustoUploadConfiguration.new(upload_concurrent_count, upload_queue_size)
  end
