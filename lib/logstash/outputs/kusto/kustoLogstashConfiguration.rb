    ...
    module KustoInternal
      class KustoLogstashConfiguration
        ...
        def validate_config
          ...
          if @kusto_proxy.proxy_aad_only && @kusto_proxy.is_direct_conn
            @logger.error('proxy_aad_only is true, but proxy parameters (Host,Port,Protocol) are missing.',
                          @kusto_proxy.proxy_host, @kusto_proxy.proxy_port, @kusto_proxy.proxy_protocol)
            raise LogStash::ConfigurationError,
                  'proxy_aad_only is true, but proxy parameters (Host,Port,Protocol) are missing.'
          end
      ...
      class KustoAuthConfiguration
        def initialize(app_id, app_key, app_tenant, managed_identity_id, cli_auth)
          ...
          @is_system_assigned_managed_identity = @is_managed_identity && managed_identity_id.to_s.strip.length > 0 && 0 == 'system'.casecmp(managed_identity_id.to_s)
        end
