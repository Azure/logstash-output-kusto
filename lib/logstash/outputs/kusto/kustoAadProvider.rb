...
def initialize(ingest_url,app_id, app_key, app_tenant, logger, proxy_host , proxy_port , proxy_protocol)
    scope = CGI.escape(sprintf("%s/.default",ingest_url))
    @logger = logger
    @aad_uri = "https://login.microsoftonline.com"
    @token_request_body = sprintf("client_id=%s&scope=%s&client_secret=%s&grant_type=client_credentials", app_id, scope, app_key)
    @token_request_uri = sprintf("%s/%s/oauth2/v2.0/token", @aad_uri, app_tenant)
    @token_state = {
    : access_token => nil,
    : expiry_time => nil,
...
# Create REST request header
    ...
    while true
        begin
            proxy_aad = sprintf("%s://%s:%s", @proxy_protocol, @proxy_host, @proxy_port)
            # Post REST request
