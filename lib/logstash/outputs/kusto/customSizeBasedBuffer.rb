      ...
      module CustomSizeBasedBuffer
        ...
        def buffer_initialize(options = {})
          unless self.class.method_defined?(:flush)
            raise ArgumentError,
                  'Any class including Stud::Buffer must define a flush() method.'
          end
        ...
        def buffer_full?
          ...
          @buffer_config[:logger].debug('---------------Entering buffer_full?-----------------') if c1 || c2
          ...
          @buffer_config[:logger].debug('---------------Exiting buffer_full?-----------------') if c1 || c2
        ...
        def buffer_flush(options = {})
          ...
          begin
            ...
            @buffer_state[:outgoing_items].each do |group, events|
              ...
              # Decrement outgoing counters for the failed group to prevent permanent buffer_full? blocking
              failed_size = events.size
              @buffer_state[:outgoing_count] -= failed_size
              if @buffer_config[:flush_each] != 0
                events_volume = 0
                events.each { |event| events_volume += var_size(event) }
                @buffer_state[:outgoing_size] -= events_volume
              end
        ...
        def process_orphaned_processing_files
          Dir.glob(::File.join(@file_persistence.failed_dir,
                               'failed_batch_*.json.processing')).each do |processing_file|
