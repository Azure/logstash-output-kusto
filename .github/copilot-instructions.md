...
- **Dynamic field references** — Logstash `%{field}` references are allowed in `path` (for time-based file rotation). The unreleased dynamic-routing feature also allows them in `database`, `table`, and `json_mapping` for queued ingestion only (one output fanning out to multiple Azure Data Explorer destinations). Dynamic routing turns on automatically when any of `database`/`table`/`json_mapping` contains a `%{...}` reference (or via `dynamic_event_routing => true`); combining it with streaming ingestion is rejected at startup. Resolved values are reversibly percent-encoded into the temp file name; unroutable events go to the Dead Letter Queue when enabled, otherwise are dropped with warnings.
