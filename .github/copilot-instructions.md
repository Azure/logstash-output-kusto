...
- **Dynamic field references** — Logstash `%{field}` references are allowed in `path` (for time-based file rotation) and, since 2.2.0, in `database`, `table`, and `json_mapping` to enable per-event dynamic routing (one output fanning out to multiple Azure Data Explorer destinations). Dynamic routing turns on automatically when any of `database`/`table`/`json_mapping` contains a `%{...}` reference (or via `dynamic_event_routing => true`). Resolved values are reversibly percent-encoded into the temp file name; unroutable events go to the Dead Letter Queue when enabled, otherwise are dropped with warnings.
